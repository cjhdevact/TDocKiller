...
echo        12  退出
echo.
echo 注意：如果你已经安装了Userinit级自动启动或者任务计划级自启动，那么它们将会覆盖当前设置，并且只支持启动到右边。要使当前设置生效，请删除Userinit级自动启动或任务计划级自启动。
echo ========================================================
echo.
...
echo ====================================================
echo.
echo 注意：安装了任务计划级级自动启动后，那么任务计划级自动启动将会覆盖Explorer的Run下的自动启动项，并且只支持启动到右边。否则如果要自定义启动位置请删除任务计划级自动启动。
schtasks.exe /Delete /TN \CJH\TDocKiller /F
schtasks.exe /create /tn \CJH\TDocKiller /xml "%~dp0TDocKiller.xml"
echo 添加成功，任意键返回... & pause > nul
goto main
