::/*****************************************************\
::
::     TDocKiller - 5-删除Userinit级自动启动该程序.bat
::
...
@echo off
cls
title 一键关闭课件小工具删除Userinit级自动启动该程序
...
echo.
echo ====================================================
echo    一键关闭课件小工具删除Userinit级自动启动该程序
echo ====================================================
echo.
echo 你可以使用以下参数：
echo 5-删除Userinit级自动启动该程序.bat [/noadm ^| /mshtaadm ^| /psadm]
echo.
...
echo.
echo 注意：安装了Userinit级自动启动后，那么Userinit级自动启动将会覆盖Explorer的Run下的自动启动项，并且只支持启动到右边。否则请删除Userinit级自动启动。
echo.
echo 任意键开始删除... & pause >nul



::set path=%1
::set path=%path:"=%
set mpath=%programfiles%\CJH\TDocKiller\TDocKiller.exe
::echo %path%
